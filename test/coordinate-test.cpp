...
TEST(coord, localCoord) {
  auto i = Vector3::Random_Unit(), j = Vector3::Random_Unit(),
       k = Vector3::Random_Unit();
  ...
  auto a = random_double(-10, 10), b = random_double(-10, 10),
       c = random_double(-10, 10);
  Point3 plocal{a, b, c};
...
TEST(planeCoord, rayIntersection) {
  auto deprecated_rayIntersectionTimeWithPlane =
      [](const Ray& r, const Point3& Q, const Vector3& u, const Vector3& v) {
        double tup, tdown;
  ...
  Coordinate3 frame = Coordinate3()
                          .Set_Translation(Coordinate3::Origin(Q))
                          .Set_Rotation(Coordinate3::AlignXY(u, v));
