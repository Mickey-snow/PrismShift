...
TEST(triangleScene, rayintersection){
  ...
  ASSERT_TRUE(rec.hits); EXPECT_EQ(rec.position, to);
  ...
  world.Add(std::make_shared<Plane>(Point3(0,-5.001,0),Vector3(1,0,0),Vector3(0,0,1)));
  to = Point3(10,0,10);
  r = Ray(fr,to-fr);
  rec = world.Ray_Hit(r, Interval<double>(0.9,1.1));
  ASSERT_TRUE(rec.hits); EXPECT_EQ(rec.position,to);
