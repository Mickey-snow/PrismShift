...
TEST(MatTransformationTest, anyAxisAlign){
  // AlignXYZ rotation using Matrix can perform the change of basis transformation with any set of basis
  // unlike QuaternionTransform, which requires the basis to be normalized and mutually perpendicular
  // it has the potential to perform rotating and scaling at the same time
  // but the use of this should not be encouraged
  auto randVec = [](){ return Vector3{random_double(-1000,1000), random_double(-1000,1000), random_double(-1000,1000)}; };
  ...
  for(int _=0;_<testcases;++_){
    ...
    EXPECT_EQ(local.Dot(basis), world); EXPECT_EQ(tr.Undo(local), world);
  }
}
