  ...
  testing::AssertionResult CheckIntersect(const AABB& box,
    ...
    if (box.isHitIn(r, interval))
      ...
      return testing::AssertionFailure()
             << "Ray r=" << r << " missed bbox b=" << box
             << " in time interval " << interval;
  }
};
...
TEST(aabbComparer, canSortAABBs) {
  AABB a(Point3(-1, 0, 0), Point3(5, 0, 0)),
      b(Point3(-2, 0, 0), Point3(4, 0, 0)), c(Point3(4, 0, 1), Point3(0, 0, 0));
...
class aabbTransformTest : public TestWithParam<MatrixTransformation> {
 protected:
  ...
  testing::AssertionResult is_transformed_inside(const std::vector<Point3>& v,
    ...
    return testing::AssertionSuccess() << "All points are inside the bbox "
                                          "before and after the transformation";
  }
  testing::AssertionResult is_transformed_outside(const std::vector<Point3>& v,
                                                  AABB box) {
    const MatrixTransformation tr = GetParam();
    for (const auto& it : v)
      if (box.Contains(it))
    ...
    return testing::AssertionSuccess() << "All points are outside the bbox "
                                          "before and after the transformation";
  }
};
...
INSTANTIATE_TEST_SUITE_P(aabbtr,
                         aabbTransformTest,
