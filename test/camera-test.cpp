...
TEST(CameraTest, DefaultInitialization) {
  ...
  EXPECT_DOUBLE_EQ(cam.aspectRatio(), 16.0 / 9.0);
...
TEST(CameraTest, SetAspectRatioRecomputesDimensions) {
  ...
  EXPECT_DOUBLE_EQ(cam.aspectRatio(), 4.0 / 3.0);
...
TEST(CameraTest, InitializeViewCustomOrientationAndFOV) {
  ...
  EXPECT_DOUBLE_EQ(view.focal_length, 1.0);
  // viewport_height = 2 * tan(90/2°) * f = 2 * tan(45°) = 2
  EXPECT_NEAR(view.viewport_height, 2.0, 1e-9);
  // viewport_width = viewport_height * (width/height) = 2 * (100/100) = 2
  EXPECT_NEAR(view.viewport_width, 2.0, 1e-9);

  // pixel deltas should be viewport dims / 100
  EXPECT_NEAR(view.pixel_delta_u.x(), -view.viewport_width / 100.0, 1e-9);
  EXPECT_NEAR(view.pixel_delta_v.y(), -view.viewport_height / 100.0, 1e-9);
}
...
TEST(CameraTest, Pixel00LocationRelativeToUpperLeft) {
  ...
  EXPECT_NEAR(diff.x(), 0.5 * deltaSum.x(), 1e-9);
  EXPECT_NEAR(diff.y(), 0.5 * deltaSum.y(), 1e-9);
  EXPECT_NEAR(diff.z(), 0.5 * deltaSum.z(), 1e-9);
}
