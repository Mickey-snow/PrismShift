...
Vector3 Ray::Scatter_Direction(const Normal& n) const{
  Vector3 normal = (Vector3)n;
  Vector3 scattered = normal + Vector3::Random_Unit();
  return scattered.Near_Zero() ? normal : scattered;
}

Vector3 Ray::Reflect_Direction(const Normal& n) const{
  Vector3 normal = (Vector3)n;
  Vector3 direction = Vector3::Normalized(this->direction);
  Vector3 scaled_normal = normal * (-Vector3::Dot(direction, normal));
  return direction + 2*scaled_normal;
}

Vector3 Ray::Refract_Direction(const Normal& n, const double& refractive_index_ratio) const{
  Vector3 normal = (Vector3)n;
  Vector3 direction = Vector3::Normalized(this->direction);
