...
bxdfSample Conductor_BRDF::Sample_f(const Vector3& wi)const{
  ...
  if(isSpecular) return BxDF::Make_Sample(col, wo, 1.0, this);

  auto delta_w = Spawn_cosine_distributed_hemisphere();
  return BxDF::Make_Sample(col, wo+delta_w*fuzz, pdf_cosine_distributed_hemisphere(delta_w), this);
}
