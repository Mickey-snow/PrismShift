...
std::vector<std::shared_ptr<BxDF>> Metal::CalculateBSDF(const Hit_record& rec){
  ...
  public:
    SpecularMetal(const Normal& norm, const Color& col) : BxDF(BxDFType::Reflection|BxDFType::Specular),normal(norm),color(col) {}
    ...
    std::tuple<Color,Vector3,double> Sample_f(const Vector3& rin) const override{
      ...
      const Vector3 out_direction = Ray(Point3(0,0,0), rin).Reflect_Direction(normal);
