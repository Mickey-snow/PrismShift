...
using tVector3 = develop::Vector3;
using tVector2 = develop::Vector2;

testing::AssertionResult Vec3eq(tVector3 vec, double x, double y,double z){
...
testing::AssertionResult Vec3eq(tVector3 vec1, tVector3 vec2){
  return Vec3eq(vec1, vec2.x(), vec2.y(), vec2.z());
}

testing::AssertionResult Vec2eq(tVector2 vec, double x,double y){
  static const double EPS = 1e-4;
...
testing::AssertionResult Vec2eq(tVector2 vec1, tVector2 vec2){
...
TEST(vec3, plus){
  tVector3 a(42,-12,4), b(0.4,321,8.9);
  tVector3 sum(42.4, 309, 12.9);
  EXPECT_TRUE(Vec3eq(a+b,sum));
}

TEST(vec3, crossproduct){
  tVector3 a(1,0,0),b(0,0,1);
  tVector3 cross_product = tVector3::Cross(a,b);
